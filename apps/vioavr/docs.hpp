...
static constexpr DocEntry DOCS[] = {
    ...
    {
        "info",
        "Display detailed device descriptor information for a specific MCU",
        "The 'info' subcommand prints the full device descriptor for a given MCU,\n"
        "including memory sizes, interrupt vectors, I/O register layout, and\n"
        ...
        "\n"
        "Shows:\n"
        "  • Flash size, SRAM size, EEPROM size\n"
        "  • Interrupt vectors and vector size\n"
        "  • I/O register layout (address ranges)\n"
        "  • Peripherals available (timers, UART, SPI, TWI, ADC, etc.)\n"
        "  • Port pin mapping\n"
        "\n"
        "The MCU name is case-insensitive.\n"
        "\n"
        "Examples:\n"
        "  vioavr info ATmega328P\n"
        "  vioavr info atmega4809\n"
        "\n"
        "For a list of all supported devices use 'vioavr list-devices'.\n"
    },
    {
        "list-devices",
        "List all supported MCU devices with optional filtering",
        "Usage:\n"
        "  vioavr list-devices [options]\n"
        "\n"
        "Options:\n"
        "  --filter <text>     Case-insensitive substring match on device name\n"
        "  --color <mode>      Color mode: auto, always, never (default: auto)\n"
        "\n"
        "Output:\n"
        "  Alphabetically sorted list of device names matching the filter,\n"
        "  or all devices if no filter is specified.\n"
        "\n"
        "Over 290 devices are supported across all families (classic AVR,\n"
        "megaAVR-0, tinyAVR-0/1/2, AVR-Dx/Ex/Lx/Sx, XMEGA).\n"
        "\n"
        "Examples:\n"
        "  vioavr list-devices\n"
        "  vioavr list-devices --filter mega\n"
        "  vioavr list-devices --filter attiny\n"
        "\n"
        "For detailed information about a specific device, use:\n"
        "  vioavr info <device>\n"
    },
    {
        "debug",
        "Interactive debugger with REPL, breakpoints, and disassembly",
        "The 'debug' subcommand starts an interactive REPL debugger for stepping\n"
        "through firmware, inspecting CPU state, setting breakpoints, and\n"
        "disassembling flash memory.\n"
        "\n"
        "Usage:\n"
        "  vioavr debug [options] <hex_file>\n"
        "\n"
        "Options:\n"
        "  --mcu <name>        MCU model (default: ATmega328P)\n"
        "  --color <mode>      Color: auto, always, never (default: auto)\n"
        "\n"
        "The REPL prompt shows the current program counter:\n"
        "  vioavr-debug:0xPC$\n"
        "\n"
        "REPL commands:\n"
        "  regs / r                  Show registers (R0-R31, SREG, SP, PC)\n"
        "  pins                      Show GPIO pin states\n"
        ...
        "  break clear <id>          Remove breakpoint by ID\n"
        "  mem <addr> [bytes] / x    Dump data space memory\n"
        "  disasm [addr] [N] / d     Disassemble N instructions from addr\n"
        "  trace on/off              Toggle instruction tracing\n"
        "  stats / status            Show CPU stats (cycles, IPC, RSS, JIT)\n"
        "  history / backtrace       Show execution history\n"
        "  help / h                  Show REPL help\n"
        "  quit / q / exit           Exit debugger\n"
        "\n"
        "Breakpoints halt execution when the program counter matches a\n"
        "set address. When continuing with breakpoints active, the debugger\n"
        "single-steps to check for breakpoint hits after each instruction.\n"
        "\n"
        "The debugger integrates with TraceBuffer via DebugTraceHook,\n"
        "recording instruction trace and interrupt events that can be\n"
        "viewed with the 'history' or 'backtrace' command.\n"
    },
    {
        "arduino",
        "Arduino sketch runner with board support and serial monitor",
        "Overview:\n"
        "  Runs Arduino sketches (.hex files compiled from Arduino IDE or arduino-cli)\n"
        "  on a simulated AVR microcontroller with board-specific pin mappings.\n"
        "\n"
        "Usage:\n"
        "  vioavr arduino <action> [options]\n"
        "\n"
        "Sub-actions:\n"
        "  list               List available Arduino boards (31 supported)\n"
        "  info <board>       Show details about a specific board\n"
        "  run <sketch.hex>   Run a compiled Arduino sketch\n"
        "\n"
        "Options for 'run':\n"
        "  --board <name>        Arduino board (default: Uno)\n"
        "  --max-cycles <n>      Stop after N cycles (suffix: k, M)\n"
        "  --show-state          Show GPIO state after run\n"
        "  --serial <mode>       Serial mode: stdio (default) or pty\n"
        "  --board-options <kv>  Comma-separated board variant options\n"
        "                          (e.g. cpu=16MHzatmega328)\n"
        "  --gdb <port>          Start GDB stub alongside simulation\n"
        "  --color <mode>        Color: auto, always, never\n"
        "\n"
        "PTY note:\n"
        "  Uses posix_openpt on Linux/macOS to create a pseudoterminal for\n"
        "  connecting with picocom or screen. Falls back to stdio on Windows.\n"
        "\n"
        "GDB note:\n"
        "  Runs simulation in chunks to interleave serial I/O with debugging.\n"
        "  Works with or without --serial.\n"
        "\n"
        "Board options:\n"
        "  Appended to FQBN via colon for a single option, comma for multiple.\n"
        "  Example: --board-options cpu=16MHzatmega328\n"
        "\n"
        "Supported boards: 31 Arduino boards (Uno, Nano, Mega, Leonardo, NG, etc.)\n"
        "\n"
        "Examples:\n"
        "  vioavr arduino list\n"
        "  vioavr arduino info uno\n"
        "  vioavr arduino run sketch.hex --board uno --serial pty\n"
        "  vioavr arduino run sketch.hex --board uno --gdb 1234\n"
        "  vioavr arduino run sketch.hex --board ng --board-options cpu=16MHzatmega8\n"
    },
    {
        "build",
        "Building VioAVR from source — prerequisites, configuration, and build commands",
        "Prerequisites:\n"
        "  • CMake 3.15+\n"
        "  • C++20 compiler (GCC 11+ / Clang 14+)\n"
        "  • Python 3 (for device descriptor generation)\n"
        "  • avr-gcc, avr-libc, and binutils-avr (for firmware tests)\n"
        "\n"
        "Quick start:\n"
        ...
        "\n"
        "Build types:\n"
        "  Release  — optimized, JIT enabled (default)\n"
        "  Debug    — no optimization, assertions enabled, for development\n"
        "\n"
        "Key CMake options:\n"
        "  -DVIOAVR_USE_NATIVE_ARCH=OFF   Disable arch-specific flags (CI)\n"
        "  -DVIOAVR_BUILD_COSIM=OFF       Disable ngspice co-simulation\n"
        "  -DVIOAVR_ENABLE_COVERAGE=ON    Enable code coverage (Debug only)\n"
        "  -DVIOAVR_ALLOW_ANY_GENERATOR=ON Allow generators other than Ninja\n"
        "\n"
        "Running tests:\n"
        "  ctest --test-dir build -j$(nproc) --output-on-failure\n"
        "\n"
        "Building Arduino sketches requires the avr-gcc toolchain.\n"
        "\n"
        "Install:\n"
    ...
    {
        "uart",
        "UART/USART peripheral \u2014 register model, interrupts, and serial I/O",
        "VioAVR models three generations of AVR UART peripherals:\n"
        "\n"
        "  UART      Classic AVR (ATmega328P, etc.) \u2014 full-duplex, 8 data bits\n"
        "  USART     Enhanced USART (ATmega4809, AVR-Dx/Ex) \u2014 frame formats\n"
        "  UART8X    megaAVR-0, tinyAVR-0/1/2 \u2014 unified memory map, 8x oversampling\n"
        "\n"
        "\u2014 Register Overview (Classic AVR) \u2014\n"
        "\n"
        "  UDR (DATA)     USART I/O Data Register \u2014 read/write data\n"
        "  UCSR0A (STATUS)  Status register + flags\n"
        "  UCSR0B (CTRL)    Control register + interrupt enables\n"
        "  UCSR0C (CTRL)    Frame format: data bits, parity, stop bits\n"
        "  UBRR (BAUD)      Baud rate register (16-bit for classic, 12-bit for UART8X)\n"
        "\n"
        "\u2014 Status Flags (UCSR0A) \u2014\n"
        "\n"
        "  RXC   Receive Complete \u2014 set when unread data in UDR\n"
        "  TXC   Transmit Complete \u2014 set when last data shifted out\n"
        "  UDRE  Data Register Empty \u2014 set when UDR ready for new data\n"
        "  FE    Framing Error \u2014 set when stop bit missing\n"
        "  DOR   Data Overrun \u2014 set when new data arrives before previous read\n"
        "  PE    Parity Error \u2014 set when parity check fails\n"
        "  U2X   Double Speed \u2014 when set, baud rate = F_CPU / (8 * UBBR + 8)\n"
        "\n"
        "\u2014 Interrupt Enables (UCSR0B) \u2014\n"
        "\n"
        ...
        "\u2014 Frame Format (UCSR0C) \u2014\n"
        "\n"
        "  Data bits:  5, 6, 7, 8, or 9 (UCSZ[2:0])\n"
        "  Parity:     None (UPM=00), Even (UPM=10), Odd (UPM=11)\n"
        "  Stop bits:  1 (USBS=0) or 2 (USBS=1)\n"
        "\n"
        ...
        "\n"
        "  Asynchronous Double Speed (U2X=1):\n"
        "    UBRR = F_CPU / (8 * BAUD) - 1\n"
        "    BAUD = F_CPU / (8 * (UBRR + 1))\n"
        "\n"
        "\u2014 C API Functions \u2014\n"
        "\n"
        "  vioavr_uart_inject_rx(handle, byte)   Inject a byte into UART RX\n"
        "  vioavr_uart_can_inject_rx(handle)     Check if RX injection is possible\n"
        "  vioavr_uart_has_tx_byte(handle)       Check if UART has transmitted a byte\n"
        "  vioavr_uart_read_tx_byte(handle)      Read the last transmitted byte\n"
        "\n"
        "  These operate on the first hardware UART of the selected MCU.\n"
        "  They return false / error codes gracefully when no UART is present.\n"
        "\n"
        "\u2014 Serial Monitor \u2014\n"
        "\n"
        "  Use 'vioavr run --serial pty' to create a pseudoterminal for connecting\n"
        "  with picocom, screen, or minicom:\n"
        "\n"
        "    vioavr run firmware.hex --mcu ATmega328P --serial pty\n"
        "    picocom /dev/pty/X\n"
        "\n"
        "  The pty mode forwards stdin/stdout through the simulated UART.\n"
        "  Falls back to stdio mode if pty creation fails.\n"
        "\n"
        "\u2014 Examples \u2014\n"
        "\n"
        "  Classic 8N1 at 9600 baud (ATmega328P, 16 MHz):\n"
        "    UCSR0C = 0x06;                     // 8 data, no parity, 1 stop\n"
        "    UBRR0  = 103;                      // 16000000 / (16 * 9600) - 1\n"
        "    UCSR0B = 0x18;                     // enable RX + TX\n"
        "\n"
        "  Double-speed 8N1 at 115200 baud:\n"
        "    UCSR0A = 0x02;                     // U2X = 1\n"
        "    UCSR0C = 0x06;                     // 8N1\n"
        "    UBRR0  = 16;                       // 16000000 / (8 * 115200) - 1\n"
        "    UCSR0B = 0x18;                     // enable RX + TX\n"
    },
    {
        "twi",
        "TWI/I2C peripheral — Two-Wire Interface master/slave operation",
        "The Two-Wire Interface (TWI) is a bidirectional serial bus compliant with\n"
        "the I2C protocol. VioAVR supports two TWI hardware variants:\n"
        "\n"
        "  • TWI   — Classic AVR (ATmega, ATtiny, AT90)\n"
        "  • TWI8X — megaAVR-0, tinyAVR-0/1/2\n"
        "\n"
        "Registers (classic TWI):\n"
        "  TWDR   TWI Data Register — shift register for transmit/receive data\n"
        "  TWAR   TWI Address Register — own slave address (7-bit, bit 0 = TWGCE)\n"
        "  TWSR   TWI Status Register — high 5 bits = status code, low 2 bits = TWPS\n"
        "  TWBR   TWI Bit Rate Register — SCL frequency divisor\n"
        "  TWCR   TWI Control Register — control and status bits\n"
        "  TWAMR  TWI Address Mask Register — address mask for multi-address match\n"
        "\n"
        "TWCR bit definitions:\n"
        "  TWINT  Interrupt flag — set when operation completes, cleared by writing 1\n"
        "  TWEA   Enable Acknowledge — enables ACK generation\n"
        "  TWSTA  START condition — set to initiate a START\n"
        "  TWSTO  STOP condition — set to initiate a STOP\n"
        "  TWWC   Write Collision — set when TWDR written while TWINT is low\n"
        "  TWEN   TWI Enable — enables the TWI peripheral\n"
        "  TWIE   TWI Interrupt Enable — enables TWI interrupt requests\n"
        "\n"
        "Bus signals:\n"
        "  SCL    Serial Clock — open-drain, pulled high with external resistor\n"
        "  SDA    Serial Data — open-drain, pulled high with external resistor\n"
        "\n"
        "Master operations:\n"
        "  1. Write TWBR for desired SCL frequency\n"
        "  2. Set TWSTA + TWEN in TWCR to generate START\n"
        "  3. Load TWDR with slave address + R/W bit\n"
        "  4. Wait for TWINT, check TWSR for ACK (0x18 or 0x40)\n"
        "  5. Transmit/receive data bytes via TWDR\n"
        "  6. Set TWSTO in TWCR to generate STOP\n"
        "\n"
        "Slave operations:\n"
        "  1. Load TWAR with own slave address\n"
        "  2. Set TWEA + TWEN in TWCR\n"
        "  3. Wait for TWINT — address match detected (0x60 or 0xA8)\n"
        "  4. Receive or transmit data as directed by R/W bit\n"
        "\n"
        "Clock rate generation:\n"
        "  SCL_freq = F_CPU / (16 + 2 * TWBR * 4 ^ TWPS)\n"
        "  where TWPS is the prescaler value in TWSR[1:0]\n"
        "\n"
        "Status codes (TWSR high 5 bits):\n"
        ...
        "  0x68  Own SLA+W received, ACK returned (arbitration lost)\n"
        "  0x70  General call address received, ACK returned\n"
        "  0x78  General call address received, ACK returned (arbitration lost)\n"
        "  0x80  Data byte received (slave), ACK returned\n"
        "  0x88  Data byte received (slave), NACK returned\n"
        "  0x90  General call data byte received, ACK returned\n"
        "  0x98  General call data byte received, NACK returned\n"
        "  0xA0  STOP or repeated START received in slave mode\n"
        "  0xA8  SLA+R received, ACK returned\n"
        "  0xB0  SLA+R received, ACK returned (arbitration lost)\n"
        "  0xB8  Data byte transmitted (slave), ACK received\n"
        "  0xC0  Data byte transmitted (slave), NACK received\n"
        "  0xC8  Last data byte transmitted, ACK received\n"
        "  0xF8  No relevant state information (idle)\n"
        "  0x00  Bus error (illegal START/STOP)\n"
        "\n"
        "Multi-master arbitration:\n"
        "  Multiple masters can share the bus. Arbitration loss is detected when\n"
        "  the driven SDA level does not match the bus level and is reported as\n"
        "  status code 0x38. The bus state is monitored via TWSR.\n"
        "\n"
        "Master write sequence example:\n"
        "  TWCR = (1<<TWSTA)|(1<<TWEN)        // START\n"
        "  TWDR = (slave_addr << 1) | 0       // SLA+W\n"
        "  TWCR = (1<<TWINT)|(1<<TWEN)        // clear TWINT, send\n"
        "  // wait for TWINT, check for 0x18\n"
        "  TWDR = data_byte\n"
        "  TWCR = (1<<TWINT)|(1<<TWEN)        // send data\n"
        "  // wait for TWINT, check for 0x28\n"
        "  TWCR = (1<<TWINT)|(1<<TWSTO)|(1<<TWEN)  // STOP\n"
        "\n"
        "Note: VioAVR supports TWI loopback test for verifying firmware\n"
        "communication between TWI peripherals.\n"
    },
    {
        "spi",
        "SPI peripheral \xe2\x80\x94 Serial Peripheral Interface master/slave operation",
        "VioAVR supports two SPI peripheral variants:\n"
        "  SPI     Classic AVR (ATmega328P, ATtiny, etc.) \xe2\x80\x94 3-register interface\n"
        "  SPI8X   megaAVR-0, tinyAVR-0/1/2 \xe2\x80\x94 AVR8X unified map (5-register)\n"
        "\n"
        "Register overview (classic SPI):\n"
        "  SPDR   SPI Data Register \xe2\x80\x94 transmit/receive data\n"
        "  SPSR   SPI Status Register \xe2\x80\x94 flags and double-speed\n"
        "  SPCR   SPI Control Register \xe2\x80\x94 mode and clock configuration\n"
        "\n"
        "Register overview (SPI8X):\n"
        "  CTRLA    Control register A (enable, master, prescaler, CPOL/CPHA)\n"
        "  CTRLB    Control register B (buffer mode, SS, etc.)\n"
        "  INTCTRL  Interrupt control\n"
        "  INTFLAGS Interrupt flags (IF, WRCOL)\n"
        "  DATA     Data register (combined SPDR with buffered TX)\n"
        "\n"
        "SPCR bits (classic AVR):\n"
        ...
        "\n"
        "SPSR bits (classic AVR):\n"
        "  SPIF   SPI Interrupt Flag (transfer complete)\n"
        "  WCOL   Write Collision (write to SPDR while transfer in progress)\n"
        "  SPI2X  Double SPI Speed\n"
        "\n"
        "SPI8X INTFLAGS bits:\n"
        "  IF     Interrupt Flag (transfer complete)\n"
        "  WRCOL  Write Collision\n"
        "\n"
        "Master clock rates (classic AVR, f_osc = system clock):\n"
        "  SPI2X  SPR1  SPR0  Clock Rate\n"
        "    0      0     0    f_osc / 4\n"
        "    0      0     1    f_osc / 16\n"
        "    0      1     0    f_osc / 64\n"
        "    0      1     1    f_osc / 128\n"
        ...
        "    1      1     1    f_osc / 64\n"
        "\n"
        "SPI8X master clock rates (PRESC field in CTRLA bits 3:1):\n"
        "  PRESC  Clock Rate\n"
        "   0     f_osc / 4\n"
        "   1     f_osc / 16\n"
        "   2     f_osc / 64\n"
        ...
        "   6     f_osc / 32\n"
        "   7     f_osc / 64\n"
        "  CTRLA bit 7 halves the divisor when set.\n"
        "\n"
        "Modes:\n"
        "  Master (MSTR=1):  SCK generated by MCU, SS optionally driven\n"
        "  Slave  (MSTR=0):  SCK driven by external master, SS selects chip\n"
        "\n"
        "Clock Polarity (CPOL):\n"
        "  0  SCK idle LOW, active HIGH\n"
        "  1  SCK idle HIGH, active LOW\n"
        "\n"
        "Clock Phase (CPHA):\n"
        ...
        "\n"
        "Data Order:\n"
        "  DORD=0  MSB-first (most significant bit transmitted first)\n"
        "  DORD=1  LSB-first (least significant bit transmitted first)\n"
        "\n"
        "SPI Interrupt:\n"
        "  Triggered when the Interrupt Flag (SPIF / IF) is set, indicating\n"
        "  a transfer is complete. In classic AVR, SPIF is cleared by reading\n"
        "  SPSR while SPIF is set, followed by a read or write of SPDR.\n"
        "  In SPI8X, IF is cleared by writing 1 to the IF bit in INTFLAGS\n"
        "  or by reading the DATA register.\n"
        "\n"
        "Typical Master Write Sequence (classic AVR):\n"
        "  1. Write data to SPDR (starts the transfer)\n"
        "  2. Wait until SPIF=1 (poll or interrupt)\n"
        "  3. Read SPDR to clear SPIF and get received byte\n"
        "\n"
        "Typical Master Write Sequence (SPI8X):\n"
        "  1. Write data to DATA register\n"
        "  2. Wait until IF=1 in INTFLAGS (poll or interrupt)\n"
        "  3. Read DATA to clear IF and get received byte\n"
        "  SPI8X data writes are buffered: a second byte can be written\n"
        "  while the first is still shifting, enabling continuous transfer.\n"
        "\n"
        "SPI8X Event System support:\n"
        "  SPI8X can receive events from the EVSYS to trigger transfers,\n"
        "  enabling peripheral-chained operation without CPU intervention.\n"
        "\n"
        "Loopback testing:\n"
        "  For testing without an external slave, connect MISO to MOSI\n"
        "  externally or use a board-level loopback.\n"
    },
    {
        "list",
