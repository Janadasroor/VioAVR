...
def parse_atdf(file_path):
    ...
    for module in root.findall(".//module"):
        ...
        for instance in module.findall("instance"):
            ...
            # Registers (from register-groups in module or instances)
            # ATDF structure can be complex, registers are defined in <module><register-group>
            # and instances refer to them.
            
            data['peripherals'][inst_name] = periph_data

    # Map registers to peripherals properly
    for module in root.findall(".//module"):
        for reg_group in module.findall(".//register-group"):
            group_name = reg_group.attrib.get('name')
            # Find which instance uses this group
            for inst_name, p_data in data['peripherals'].items():
                if inst_name == group_name or inst_name in group_name: # Simple heuristic
                    for reg in reg_group.findall("register"):
                        ...
                        for bf in reg.findall("bitfield"):
                            r_data['bitfields'][bf.attrib.get('name')] = {
                                'mask': int(bf.attrib.get('mask', '0'), 16),
                                'caption': bf.attrib.get('caption')
                            }
                        p_data['registers'][reg_name] = r_data
