...
def parse_atdf(file_path):
    ...
    root = tree.getroot()
    device_info = {'name': device_name, 'flash_words': 0, 'sram_bytes': 0, 'eeprom_bytes': 0, 'vectors': 0, 'vector_size': 2, 'adc': {}, 'timer0': {}, 'timer1': {}, 'uart0': {}, 'ext_interrupt': {}, 'spi': {}, 'twi': {}, 'eeprom_io': {}, 'wdt': {}, 'ports': []}
    ...
    device_info['timer0'] = { 'tcnt': fr(r'TCNT0'), 'ocra': fr(r'OCR0A|OCR0'), 'ocrb': fr(r'OCR0B'), 'tccra': fr(r'TCCR0A|TCCR0'), 'tccrb': fr(r'TCCR0B'), 'timsk': fr(r'TIMSK0|TIMSK'), 'tifr': fr(r'TIFR0|TIFR'), 'v_ovf': fv(r'TIMER0_OVF'), 'v_compa': fv(r'TIMER0_COMPA?'), 'v_compb': fv(r'TIMER0_COMPB') }
...
inline constexpr DeviceDescriptor {name} {{
    ...
    .timer0 = {{ .tcnt_address = {hx(d['timer0']['tcnt'])}, .ocra_address = {hx(d['timer0']['ocra'])}, .ocrb_address = {hx(d['timer0']['ocrb'])}, .tifr_address = {hx(d['timer0']['tifr'])}, .timsk_address = {hx(d['timer0']['timsk'])}, .tccra_address = {hx(d['timer0']['tccra'])}, .tccrb_address = {hx(d['timer0']['tccrb'])}, .compare_a_vector_index = {d['timer0']['v_compa']}U, .compare_b_vector_index = {d['timer0']['v_compb']}U, .overflow_vector_index = {d['timer0']['v_ovf']}U, .compare_a_enable_mask = 0x02U, .compare_b_enable_mask = 0x04U, .overflow_enable_mask = 0x01U }},
