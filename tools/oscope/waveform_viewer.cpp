...
// SVG/Print export not used in standalone; QSvgGenerator/QPrinter not needed
#include <cmath>
#include <algorithm>
#include <numeric>
...
void WaveformViewer::loadCsv(const QString& filePath) {
    ...
    while (!in.atEnd()) {
        ...
        if (line.contains(',')) {
            ...
            // Skip non-numeric lines (ngspice debug/notes), but keep "Index" lines
            if (parts.size() >= 1 && parts[0] != "Index") {
        ...
        if (columns.isEmpty()) {
            ...
            if (firstRowIsHeader) {
                ...
                for (int i = 0; i < parts.size(); ++i)
                    headers.append(QString(parts[i]).remove("v(").remove(")"));
                ngspiceFormat = (headers.size() >= 2 && headers[0] == "Index" && headers[1] == "time");
                if (ngspiceFormat)
                    headers = headers.mid(2);
                else
        ...
        // Skip repeated "Index" header lines after columns are populated
        if (!columns.isEmpty() && parts[0] == "Index")
            continue;
        ...
        if (ngspiceFormat) {
            ...
            bool tOk;
            double t = parts[1].toDouble(&tOk);
            if (!tOk) continue; // skip rows with invalid time

            bool rowOk = true;
            for (int c = 0; c < colCount && c + 2 < parts.size(); ++c) {
                bool ok = false;
                double val = parts[c + 2].toDouble(&ok);
                if (!ok) { rowOk = false; break; }
            }
            if (!rowOk) continue; // skip rows with any unparseable value

            timeData.append(t);
            for (int c = 0; c < colCount && c + 2 < parts.size(); ++c) {
                columns[c].append(parts[c + 2].toDouble());
            }
        } else {
    ...
    if (ngspiceFormat && !timeData.isEmpty()) {
        time = timeData;
    } else {
