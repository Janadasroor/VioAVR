...
TEST_CASE("Timer0 Dual Interrupt (Compare and Overflow) Test")
{
    ...
    SUBCASE("Simultaneous trigger") {
        bus.write_data(atmega328.timers8[0].ocra_address, 0x00U);
        bus.write_data(atmega328.timers8[0].ocrb_address, 0x80U);
        bus.write_data(atmega328.timers8[0].tcnt_address, 0xFFU);
        bus.write_data(atmega328.timers8[0].timsk_address, 0x03U); // Enable both OCIE0A and TOIE0
        ...
        CHECK(request.vector_index == atmega328.timers8[0].compare_a_vector_index);
        ...
        CHECK(bus.pending_interrupt_request(request));
        CHECK(request.vector_index == atmega328.timers8[0].overflow_vector_index);
