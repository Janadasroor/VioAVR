...
TEST_CASE("Timer0 Control and Prescaler Test")
{
    ...
    SUBCASE("Prescaler 8 (CS0[2:0]=010)") {
        ...
        bus.write_data(atmega328.timers8[0].tccrb_address, 0x02U); // clk/8
    ...
    SUBCASE("CTC Mode and Prescaler 32 (CS0[2:0]=011 -> wait, 011 is clk/64)") {
        ...
        bus.write_data(atmega328.timers8[0].tccra_address, 0x02U); // WGM01=1 (CTC)
        bus.write_data(atmega328.timers8[0].tccrb_address, 0x03U); // clk/64
        bus.write_data(atmega328.timers8[0].ocra_address, 0x01U);
        bus.write_data(atmega328.timers8[0].tcnt_address, 0x00U);
