...
TEST_CASE("ADC Basic Functionality Test")
{
    ...
    const auto adc_vector = atmega328p.adcs[0].vector_index;

    PinMux pin_mux(8);
    MemoryBus bus {atmega328p};
    Adc adc0 {"ADC0", atmega328p.adcs[0], pin_mux, 6U, 4U}; // Source ID 6, 4 cycles
    adc0.set_bus(bus);
    ...
    SUBCASE("Single Conversion") {
        ...
        // Clear ADIF
        bus.write_data(atmega328p.adcs[0].adcsra_address, 0x10U);
