...
TEST_CASE("Timer2 compare output can be wired from descriptor pin metadata")
{
    ...
    timer2.set_bus(bus);
    timer2.connect_compare_output_a(port_b, atmega328p.timer2.ocra_pin_bit);
    timer2.connect_compare_output_b(port_d, atmega328p.timer2.ocrb_pin_bit);
    ...
    const auto oc2a_mask = static_cast<vioavr::core::u8>(1U << atmega328p.timer2.ocra_pin_bit);
    bus.write_data(port_b.port_address() - 1U, oc2a_mask); // DDRB
    bus.write_data(atmega328p.timer2.ocra_address, 0x02U);
    bus.write_data(atmega328p.timer2.tccra_address, 0x42U); // COM2A toggle + WGM21 CTC
