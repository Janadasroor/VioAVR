...
TEST_CASE("Timer8 Normal Mode Overflow Timing")
{
    ...
    bus.write_data(atmega328.timers8[0].timsk_address, 0x01U); // TOIE0=1
    bus.write_data(atmega328.timers8[0].tccrb_address, 0x01U); // CS00=1 (no prescaler)

    // Run 256 ticks - should overflow exactly once
    timer0.tick(256);

    // TIFR overflow flag should be set
    CHECK((bus.read_data(atmega328.timers8[0].tifr_address) & 0x01U) != 0U);
...
TEST_CASE("Timer8 CTC Mode Compare Match Timing")
{
    ...
    bus.write_data(atmega328.timers8[0].ocra_address, 49U);
    // Enable compare match A interrupt
    bus.write_data(atmega328.timers8[0].timsk_address, 0x02U); // OCIE0A=1
    // CTC mode (WGM01=1), no prescaler
    bus.write_data(atmega328.timers8[0].tccra_address, 0x02U);
    bus.write_data(atmega328.timers8[0].tccrb_address, 0x01U);

    // Run 50 ticks - should reach OCR and trigger compare match
    timer0.tick(50);

    // TIFR compare match A flag should be set
    CHECK((bus.read_data(atmega328.timers8[0].tifr_address) & 0x02U) != 0U);
...
TEST_CASE("Timer8 CTC Mode Compare Match at OCR=0")
{
    ...
    bus.write_data(atmega328.timers8[0].ocra_address, 0U);
    bus.write_data(atmega328.timers8[0].timsk_address, 0x02U);
    bus.write_data(atmega328.timers8[0].tccra_address, 0x02U); // CTC
    bus.write_data(atmega328.timers8[0].tccrb_address, 0x01U);
    ...
    bus.write_data(atmega328.timers8[0].tifr_address, 0x02U);
    timer0.tick(1);
    CHECK((bus.read_data(atmega328.timers8[0].tifr_address) & 0x02U) != 0U);
}
...
TEST_CASE("Timer8 Fast PWM Mode Pin Toggle")
{
    ...
    bus.write_data(atmega328.ports[2].ddr_address, 1U << 6);

    // Set OCR = 128 (50% duty cycle)
    bus.write_data(atmega328.timers8[0].ocra_address, 128U);
    // Fast PWM (WGM01=1, WGM00=1), non-inverting (COM0A=2), no prescaler
    bus.write_data(atmega328.timers8[0].tccra_address, 0x83U); // COM0A=2, WGM01|WGM00=1
    bus.write_data(atmega328.timers8[0].tccrb_address, 0x09U); // WGM02=1
...
TEST_CASE("Timer8 Phase-Correct PWM Direction Change")
{
    ...
    bus.write_data(atmega328.timers8[0].tccra_address, 0x01U); // WGM00=1 -> WGM=1 = phase-correct PWM
    bus.write_data(atmega328.timers8[0].tccrb_address, 0x01U); // CS00=1
...
TEST_CASE("Timer8 Compare Match B Interrupt")
{
    ...
    bus.write_data(atmega328.timers8[0].ocrb_address, 100U);
    bus.write_data(atmega328.timers8[0].timsk_address, 0x04U); // OCIE0B=1
    bus.write_data(atmega328.timers8[0].tccrb_address, 0x01U); // CS00=1

    // Tick 100 times to reach OCRB
    timer0.tick(100);
    CHECK((bus.read_data(atmega328.timers8[0].tifr_address) & 0x04U) != 0U);

    // Interrupt request should be pending
    InterruptRequest request {};
    CHECK(bus.pending_interrupt_request(request));
    CHECK(request.vector_index == atmega328.timers8[0].compare_b_vector_index);
}
...
TEST_CASE("Timer16 Normal Mode 16-bit Overflow")
{
    ...
    bus.write_data(atmega328.timers16[0].timsk_address, 0x01U); // TOIE1=1
    bus.write_data(atmega328.timers16[0].tccrb_address, 0x01U); // CS10=1

    // Run 65536 ticks - should overflow exactly once
    timer1.tick(65536);
    CHECK((bus.read_data(atmega328.timers16[0].tifr_address) & 0x01U) != 0U);
...
TEST_CASE("Timer16 CTC Mode with ICR as TOP")
{
    ...
    bus.write_data(atmega328.timers16[0].icr_address + 1, 0x03U); // ICRH = 0x03
    bus.write_data(atmega328.timers16[0].icr_address, 0xE7U);     // ICRL = 0xE7 (0x03E7 = 999)

    bus.write_data(atmega328.timers16[0].timsk_address, 0x02U); // OCIE1A=1
    // CTC with ICR (WGM=12: WGM13=1, WGM12=1), no prescaler
    bus.write_data(atmega328.timers16[0].tccrb_address, 0x19U); // WGM13|WGM12=1, CS10=1
    bus.write_data(atmega328.timers16[0].tccra_address, 0x00U); // WGM11:0=0

    // Run 1000 ticks
    timer1.tick(1000);
    CHECK((bus.read_data(atmega328.timers16[0].tifr_address) & 0x02U) != 0U); // OCF1A set
    CHECK(timer1.counter() == 0U);
}
...
TEST_CASE("Timer16 Input Capture")
{
    ...
    bus.write_data(atmega328.timers16[0].tcnt_address + 1, 0x12U); // TCNTH
    bus.write_data(atmega328.timers16[0].tcnt_address, 0x34U);     // TCNTL
    CHECK(timer1.counter() == 0x1234U);

    // Enable input capture interrupt (ICIE1=1), rising edge (ICES1=1)
    bus.write_data(atmega328.timers16[0].timsk_address, 0x20U);
    bus.write_data(atmega328.timers16[0].tccrb_address, 0x41U); // ICES1=1, CS10=1
    ...
    bus.write_data(atmega328.timers16[0].tifr_address, 0x20U); // Clear any existing ICF1
    ...
    u8 icrl = bus.read_data(atmega328.timers16[0].icr_address);
    u8 icrh = bus.read_data(atmega328.timers16[0].icr_address + 1);
    u16 icr = static_cast<u16>(icrl) | (static_cast<u16>(icrh) << 8);
    CHECK(icr == 0U); // ICR not yet updated

    // Verify interrupt flags are clear
    CHECK((bus.read_data(atmega328.timers16[0].tifr_address) & 0x20U) == 0U);
}
...
TEST_CASE("Timer16 Fast PWM 10-bit Mode")
{
    ...
    bus.write_data(atmega328.ports[0].ddr_address, 1U << 1);

    // Set OCR = 512 (50% duty cycle)
    bus.write_data(atmega328.timers16[0].ocra_address + 1, 0x02U); // OCRH
    bus.write_data(atmega328.timers16[0].ocra_address, 0x00U);     // OCRL = 0x0200 = 512

    // Fast PWM 10-bit (WGM=7: WGM12=1, WGM11=1, WGM10=1)
    // Non-inverting (COM1A=2)
    bus.write_data(atmega328.timers16[0].tccra_address, 0x83U); // COM1A=2, WGM11|WGM10=1
    bus.write_data(atmega328.timers16[0].tccrb_address, 0x09U); // WGM12=1, CS10=1
