...
TEST_CASE("USB Double Banking Fidelity")
{
    ...
    // Clear FIFOCON+TXINI to release Bank 0 to SIE (W0C: write 0 to bits 7 and 0)
    bus.write_data(atmega32u4.usbs[0].ueintx_address, 0x7E); // ~(FIFOCON|TXINI) 
    ...
    // Clear FIFOCON+TXINI to release Bank 1 to SIE (W0C: write 0 to bits 7 and 0)
    bus.write_data(atmega32u4.usbs[0].ueintx_address, 0x7E); // ~(FIFOCON|TXINI)
