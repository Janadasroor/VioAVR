...
TEST_CASE("Pin Change Voltage Isolation Test")
{
    ...
    // Setup
    bus.write_data(atmega328p.pcints[0].pcmsk_address, 0x04U);
    bus.write_data(atmega328p.pcints[0].pcicr_address, 0x01U);

    // Initial state: check pcifr is cleared
    CHECK(bus.read_data(atmega328p.pcints[0].pcifr_address) == 0x00U);
