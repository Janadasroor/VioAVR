...
TEST_CASE("Timer0 basic functionality")
{
    ...
    bus.load_image(HexImage {
        .flash_words = {
            ...
            encode_sts(16U), atmega328p.timers8[0].ocra_address,
            encode_ldi(22U, 0x01U),  // CS00
            encode_out(0x25U, 22U),  // TCCR0B
            encode_sts(22U), atmega328p.timers8[0].tcnt_address, // Placeholder to use STS for TCNT
            // Actually let's just use the addresses from atmega328p.timers8[0]
        },
        .entry_word = 0U
    });
    ...
    std::vector<u16> code = {
        ...
        encode_sts(17U), atmega328p.timers8[0].tccrb_address,
        // Wait a few cycles
        0x0000, 0x0000, 0x0000,
        // Read TCNT0
        0x9100, atmega328p.timers8[0].tcnt_address, // LDS R16, TCNT0
        // Read TIFR0
        0x9110, atmega328p.timers8[0].tifr_address, // LDS R17, TIFR0
        0x0000
    };
    bus.load_flash(code);
    cpu.reset();
