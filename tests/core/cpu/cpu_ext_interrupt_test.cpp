...
TEST_CASE("External Interrupt Logic Test")
{
    ...
    SUBCASE("Falling Edge Trigger") {
        ...
        bus.write_data(atmega328p.ext_interrupts[0].eicra_address, 0x02U); // Falling edge (ISC0[1:0] = 10)
        ...
        CHECK(bus.read_data(atmega328p.ext_interrupts[0].eifr_address) == 0x01U);
    ...
    SUBCASE("Rising Edge Trigger") {
        bus.write_data(atmega328p.ext_interrupts[0].eimsk_address, 0x01U);
        bus.write_data(atmega328p.ext_interrupts[0].eicra_address, 0x03U); // Rising edge (ISC0[1:0] = 11)
    ...
    SUBCASE("Clear Flag by Writing 1") {
        bus.write_data(atmega328p.ext_interrupts[0].eicra_address, 0x03U);
        exti.set_int0_level(false);
        exti.set_int0_level(true);
        CHECK((bus.read_data(atmega328p.ext_interrupts[0].eifr_address) & 0x01U) != 0U);

        bus.write_data(atmega328p.ext_interrupts[0].eifr_address, 0x01U); // Write 1 to clear
        CHECK(bus.read_data(atmega328p.ext_interrupts[0].eifr_address) == 0x00U);
    ...
    SUBCASE("Low Level Trigger") {
        bus.write_data(atmega328p.ext_interrupts[0].eimsk_address, 0x01U);
        bus.write_data(atmega328p.ext_interrupts[0].eicra_address, 0x00U); // Low level (ISC0[1:0] = 00)
