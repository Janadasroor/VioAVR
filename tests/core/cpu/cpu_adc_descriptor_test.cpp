...
TEST_CASE("ADC Descriptor and Trigger Logic Test")
{
    ...
    SUBCASE("Trigger Select Register") {
        adc0.write(atmega328p.adcs[0].adcsrb_address, 0x01U);
        CHECK(adc0.trigger_select_register() == 0x01U);
    }

    SUBCASE("Auto Trigger - Comparator") {
        adc0.write(atmega328p.adcs[0].adcsrb_address, 0x01U); // Comparator trigger select
        adc0.write(atmega328p.adcs[0].adcsra_address, 0xA0U); // ADEN | ADATE
    ...
    SUBCASE("Auto Trigger - Timer Overflow") {
        adc0.reset();
        adc0.write(atmega328p.adcs[0].adcsrb_address, 0x04U); // Timer0 Overflow trigger select
        adc0.write(atmega328p.adcs[0].adcsra_address, 0xA0U);
    ...
    SUBCASE("Auto Trigger - Mismatched Source") {
        adc0.reset();
        adc0.write(atmega328p.adcs[0].adcsrb_address, 0x06U); // DIFFERENT trigger select
        adc0.write(atmega328p.adcs[0].adcsra_address, 0xA0U);
