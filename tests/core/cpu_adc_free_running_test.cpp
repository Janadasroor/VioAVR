...
TEST_CASE("ADC Free Running Mode Test")
{
    ...
    bus.write_data(atmega328.adcs[0].adcsrb_address, 0x00U);   // ADTS = free running
    bus.write_data(atmega328.adcs[0].adcsra_address, 0xE0U);   // ADEN | ADSC | ADATE
    ...
    SUBCASE("Continuous conversion") {
        ...
        bus.write_data(atmega328.adcs[0].adcsra_address, 0xF0U); // Clear ADIF
