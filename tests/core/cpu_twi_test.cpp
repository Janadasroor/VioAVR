...
TEST_CASE("TWI (I2C) Peripheral Master Mode Test")
{
    ...
    bus.load_image(HexImage {
        .flash_words = {
            ...
            encode_sts(16), atmega328p.twis[0].twcr_address,
            
            // Send SLA+W (address 0x50)
            encode_ldi(17, 0xA0), // 0x50 << 1
            encode_sts(17), atmega328p.twis[0].twdr_address,
            encode_ldi(18, 0x84), // TWINT=1, TWEN=1
            encode_sts(18), atmega328p.twis[0].twcr_address,

            // Send DATA 0xDE
            encode_ldi(19, 0xDE),
            encode_sts(19), atmega328p.twis[0].twdr_address,
            encode_sts(18), atmega328p.twis[0].twcr_address,

            // Send STOP
            encode_ldi(20, 0x94), // TWINT=1, TWSTO=1, TWEN=1
            encode_sts(20), atmega328p.twis[0].twcr_address,
    ...
    SUBCASE("Master Transmission Sequence") {
        ...
        CHECK((twi.read(atmega328p.twis[0].twsr_address) & 0xF8U) == 0x08U); // START status
        ...
        CHECK((twi.read(atmega328p.twis[0].twsr_address) & 0xF8U) == 0x18U); // SLA+W ACK status
        ...
        cpu.step(); // STS TWCR
        bus.tick_peripherals(200);
        CHECK((twi.read(atmega328p.twis[0].twsr_address) & 0xF8U) == 0x28U); // Data ACK status
        ...
        CHECK((twi.read(atmega328p.twis[0].twsr_address) & 0xF8U) == 0xF8U);
    }
}
