...
TEST_CASE("Reset State Verification")
{
    ...
    SUBCASE("Stack Pointer at RAMEND") {
        ...
        const u16 ramend = atmega328.sram_range().end;
...
TEST_CASE("Interrupt Vector Address Table Verification")
{
    ...
    SUBCASE("Vector 0 (RESET) at word 0x0000") {
        CHECK(0U * (atmega328.interrupt_vector_size / 2U) == 0U);
    }

    SUBCASE("Vector 1 (INT0) at word 0x0002") {
        CHECK(1U * (atmega328.interrupt_vector_size / 2U) == 2U);
    }

    SUBCASE("Vector 2 (INT1) at word 0x0004") {
        CHECK(2U * (atmega328.interrupt_vector_size / 2U) == 4U);
    }

    SUBCASE("Vector 6 (WDT) at word 0x000C") {
        CHECK(6U * (atmega328.interrupt_vector_size / 2U) == 12U);
    }

    SUBCASE("Vector 14 (TIMER0_COMPA) at word 0x001C") {
        CHECK(14U * (atmega328.interrupt_vector_size / 2U) == 28U);
    }

    SUBCASE("Vector 18 (USART_RX) at word 0x0024") {
        CHECK(18U * (atmega328.interrupt_vector_size / 2U) == 36U);
    }

    SUBCASE("Vector 23 (ANALOG_COMP) at word 0x002E") {
        CHECK(23U * (atmega328.interrupt_vector_size / 2U) == 46U);
    }

    SUBCASE("Vector 25 (SPM_READY) at word 0x0032") {
        CHECK(25U * (atmega328.interrupt_vector_size / 2U) == 50U);
    }

    SUBCASE("All vectors fit within flash") {
        const u16 last_vector_word = static_cast<u16>(
            (atmega328.interrupt_vector_count - 1U) * (atmega328.interrupt_vector_size / 2U));
        CHECK(last_vector_word < atmega328.flash_words);
    }
}
