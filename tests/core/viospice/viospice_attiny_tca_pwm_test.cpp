...
int main(void) {
    ...
    fs::path tmp_dir = fs::temp_directory_path() / "vioavr_attiny_tca_pwm";
    ...
    std::string compile_cmd = "avr-gcc -mmcu=attiny412 -Os -DF_CPU=16000000UL -o "
...
static bool generate_circuit(const fs::path& cir_path, const fs::path& hex_path,
    ...
    of << "* VioAVR ATtiny412 TCA PWM co-simulation test\n";
    of << "* TCA0 WO0 routed to PA3 (external ID 3)\n";
    of << "\n";
    of << "R_pull pa3_dig 0 10k\n";
...
TEST_CASE("ATtiny412 TCA PWM co-simulation") {
    ...
    fs::path test_dir = build_dir / "cosim_tests" / "attiny412_tca_pwm";
    ...
    fs::path cir_file = test_dir / "attiny412_tca_pwm.cir";
    ...
    // Check PA3 for PWM toggling (LOW < 0.1V, HIGH > 4.0V)
    bool saw_low = false, saw_high = false;
    bool saw_multiple_transitions = false;
    int data_rows = 0;
    int transitions = 0;
    int prev_bucket = -1; // 0=LOW, 1=HIGH
    std::string line;
    ...
    MESSAGE("ATtiny412 TCA PWM co-sim: " << data_rows << " rows, "
            << transitions << " transitions, "
            << (elapsed * 1000) << "ms");

    CHECK_MESSAGE(data_rows > 0, "No data rows in output");
    CHECK_MESSAGE(saw_low, "PA3 never goes LOW (<0.1V)");
    CHECK_MESSAGE(saw_high, "PA3 never goes HIGH (>4.0V)");
    CHECK_MESSAGE(saw_multiple_transitions,
                  "PA3 shows fewer than 3 transitions (expected PWM toggling)");

    MESSAGE("PASS ATtiny412 TCA PWM (" << elapsed << "s)");
}
