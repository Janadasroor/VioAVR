...
class AdcEa final : public IoPeripheral {
public:
    explicit AdcEa(const AdcEaDescriptor& desc) noexcept;
    ...
    [[nodiscard]] std::string_view name() const noexcept override { return "ADCEA"; }
...
private:
    ...
    [[nodiscard]] u8 get_resolution() const noexcept;
    [[nodiscard]] u8 get_pga_gain() const noexcept;

    void start_conversion() noexcept;
    void complete_conversion() noexcept;

    AdcEaDescriptor desc_;
    MemoryBus* bus_ {};
    ...
    double temperature_ {27.0};
    double tos_offset_ {307.0};
    double tos_slope_ {1.0};
